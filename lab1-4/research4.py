...
def lab4():
    edf([np.random.standard_normal(20),
         np.random.standard_normal(60),
         np.random.standard_normal(100)],
         sps.norm.cdf,
         np.linspace(-4, 4, 100),
         "Normal")
    edf([np.random.standard_cauchy(20),
         np.random.standard_cauchy(60),
         np.random.standard_cauchy(100)],
         sps.cauchy.cdf,
         np.linspace(-4, 4, 100),
         "Cauchy")
    edf([np.random.laplace(loc=0, scale=1.0/np.sqrt(2.0), size=20),
    ...
    for n in [20, 60, 100]:
        kde(np.random.standard_normal(n),
            sps.norm.pdf,
            np.linspace(-4, 4, 100),
            "Normal")
    for n in [20, 60, 100]:
        kde(np.random.standard_cauchy(n),
            sps.cauchy.pdf,
            np.linspace(-4, 4, 100),
            "Cauchy")
    for n in [20, 60, 100]:
        kde(np.random.laplace(loc=0, scale=1.0/np.sqrt(2.0), size=n),
